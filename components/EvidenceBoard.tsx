...
const EvidenceBoard: React.FC<EvidenceBoardProps> = ({ destination, evidenceItems }) => {
  ...
  const generateSummary = useCallback(async () => {
    if (evidenceItems.length === 0) {
      setError("Add some photos or notes to your shot list before I can give feedback!");
      return;
    }
    setIsLoading(true);
    setError('');
    ...
    try {
      ...
      console.error(e);
      setError('Failed to generate summary. We seem to have a problem in post-production.');
    } finally {
      setIsLoading(false);
    }
  ...
  return (
    <div className="bg-yellow-100 p-6 rounded-lg mb-8 shadow-lg border-2 border-yellow-300 relative overflow-hidden">
        <div className="absolute top-2 right-2 text-yellow-400 font-mono text-xs z-0">SCENE-001 SHOT LIST</div>
      <h2 className="font-heading text-3xl font-bold text-slctrips-navy mb-2 relative z-10">Director's Cut</h2>
      <p className="text-gray-700 mb-6 relative z-10">Your shots and notes appear here. When you're ready, I'll give you my director's feedback.</p>

      {/* Evidence Grid */}
      <div className="bg-yellow-50 border border-yellow-200 p-4 rounded-md min-h-[150px]">
        {evidenceItems.length === 0 ? (
          <p className="text-center text-gray-500 italic py-10">Your shot list is empty. Use the Location Log to add your first setup!</p>
        ) : (
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
            {evidenceItems.map((item, index) => (
              <div key={item.id} className="bg-white p-2 rounded shadow-md transform -rotate-2 hover:rotate-0 hover:scale-105 transition-transform duration-300">
                {item.photo && <img src={item.photo} alt={`Shot ${index + 1}`} className="rounded-sm w-full h-auto" />}
      ...
      {/* Analysis Section */}
      <div className="mt-6">
        <h3 className="font-heading text-2xl font-bold text-slctrips-red mb-4">Director's Feedback</h3>
        {!isGenerated ? (
             <button
            ...
            <div className="bg-white p-4 rounded-md border border-slctrips-mid min-h-[150px]">
                 {isLoading && !summary && <p className="text-slctrips-navy animate-pulse">Reviewing the footage...</p>}
